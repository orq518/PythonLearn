...
class PrintProgress(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            print (PRO_INFO.format(p_info['total_number'], p_info['wait_to_process'],
                                  p_info['success'], p_info['failure'], time.time() - START_TIME)).decode('utf-8')
...
class Spider(object):
    def __init__(self, url, deep=0, key=None):
        self.url = url
        self.deep = deep
        self.key = key
    ...
    def spider(self):
        '''
          抓取并保存页面, 并且根据deep返回页面链接，根据key确定是否保存该页面,其中:
          deep == 0时，是抓取的最后一层深度，即只抓取并保存页面，不分析链接
          deep > 0时，返回该页面链接
        '''
        if self.html:
            self.save_to_db()
        else: pool.failure += 1     # 假如html为空则代表爬取失败
        links = []
        if self.deep > 0:
    ...
    def save_to_db(self):
        def save():
            try:
                ...
                conn.commit()
                pool.success += 1
                logging.debug("<<<<< commit success in: %s >>>>>"%(threading.current_thread().getName()))
                #db_lock.release()
            except Exception, e:
                pool.failure += 1
...
class ThreadPool(object):
    ...
    def __init__(self, opts):
        ...
        # 已经处理过的url, 避免重复
        self.saved_urls = {}
        # 为了避免处理重复的URL, 因此用列表或者字典保存处理过的.但是由于
